...
export const DriveConnect: React.FC<DriveConnectProps> = ({ files, setFiles }) => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
       ...
       if (['txt', 'md', 'csv', 'json'].includes(fileType || '')) {
           ...
           setTimeout(() => {
                addFile(file.name, `[SIMULAÇÃO: Conteúdo extraído do arquivo PDF '${file.name}'. Em uma aplicação real, aqui estaria o texto processado do documento PDF.]`, 'pdf', 'upload');
           }, 1000);
       } else {
  ...
  const simulateIntegration = (service: string, mockFiles: DriveFile[]) => {
      ...
      setTimeout(() => {
          ...
          // Auto switch to list to show success
      }, 1500);
  };
        ...
        <div className="p-8 border-b border-gray-800 bg-gray-900/50">
             <div className="max-w-5xl mx-auto">
                ...
                <p className="text-gray-400 text-lg">
                    Conecte o cérebro da sua empresa. Adicione documentos, emails e planilhas para que os agentes tenham contexto total.
                </p>
                ...
                {activeTab === 'integrations' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-gray-600 transition-all shadow-lg group">
                            ...
                            <p className="text-gray-500 text-sm mb-6 min-h-[40px]">Conecte pastas inteiras, Docs e PDFs para pesquisa semântica.</p>
                            <button 
                                onClick={() => simulateIntegration('drive', [
                                    ...
                                    { id: 'drive-2', name: 'Planejamento_Q3.doc', type: 'doc', source: 'gdrive', content: 'Planning...', lastModified: new Date() }
                                ])}
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-gray-600 transition-all shadow-lg group">
                            ...
                            <p className="text-gray-500 text-sm mb-6 min-h-[40px]">Permita que agentes leiam newsletters ou emails de clientes.</p>
                            <button 
                                onClick={() => simulateIntegration('gmail', [
                                    { id: 'mail-1', name: 'Feedback_Cliente_X.eml', type: 'email', source: 'gmail', content: 'Email body...', lastModified: new Date() }
                                ])}
                                disabled={!!isConnecting}
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-gray-600 transition-all shadow-lg group">
                            <div className="w-12 h-12 bg-gray-800 rounded-xl flex items-center justify-center mb-4 group-hover:bg-gray-700 transition-colors">
                                <div className="w-8 h-8"><Icons.Sheet /></div>
                            </div>
                            <h3 className="text-lg font-bold mb-1">Planilhas</h3>
                            <p className="text-gray-500 text-sm mb-6 min-h-[40px]">Sincronize dados de vendas, CRM ou cronogramas.</p>
                            <button 
                                onClick={() => simulateIntegration('sheets', [
                                    { id: 'sheet-1', name: 'Vendas_2024.csv', type: 'sheet', source: 'sheets', content: 'CSV Data...', lastModified: new Date() }
                                ])}
                                disabled={!!isConnecting}
                                className="w-full py-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-sm font-medium transition-colors"
                            >
                                {isConnecting === 'sheets' ? 'Sincronizando...' : 'Conectar Planilhas'}
                            </button>
                        </div>
                    </div>
                )}
                ...
                <div className="mt-12">
                    ...
                    {files.length === 0 ? (
                        <div className="text-center py-12 text-gray-600 italic">
                            Sua base de conhecimento está vazia. Adicione arquivos para treinar seus agentes.
                        </div>
                    ) : (
                                ...
                                <div key={file.id} className="p-4 border-b border-gray-800 last:border-0 flex items-center justify-between group hover:bg-gray-800/50 transition-colors">
                                    <div className="flex items-center gap-4">
                                        <div className={`p-2 rounded-lg ${
                                            ...
                                            file.source === 'sheets' ? 'bg-green-900/20 text-green-400' :
